...
namespace DashboardApi.Controllers
{
    ...
    public class Dashboard : ControllerBase
    {
        ...
        public Dashboard(BD1Context d1Context, BD2Context bD2Context, DBRebelContext dBRebelContext, IConfiguration configuration, 
            DashboardContext dashboardcontext, MailC fn,Funciones fx) 
        {
        ...
        [HttpGet]
        [Route("getPdfSuc")]
        public async Task<ActionResult> getPdfSuc()
        {
            try
            {
                string docpdf = _funciones.GeneratePdf(); 

                return StatusCode(StatusCodes.Status200OK, new { base64File = docpdf });
            }
            catch (Exception ex)
            {
                return StatusCode(StatusCodes.Status500InternalServerError, new { message = ex.ToString() });
        ...
        public async Task<ActionResult> get25pts(string sucursales, string fechaini, string fechafin)
        {
            try
            {
                ...
                foreach (int ids in listsuc) 
                {
                    ...
                    if (sucursal != null) 
                    {
                        var datas = _dbRebelContext.It25pts.Where(x => x.Fechaini.Value.Date >= fechaInicial.Date && x.Fechaini.Value.Date <= fechaFinal.Date && x.Sucursal == sucursal.Titulo && !x.Justificacion.Equals("prueba")).ToList();
                        if(datas.Count>0) 
                        {
                            data.AddRange(datas); 
                        }
                        ...
                        var vendedores =datas
                                           .Select(v => v.Vendedor) // Seleccionar los nombres
                        ...
                        foreach (string vend in vendedores) 
                        {
                            var vendedor = vendedoressuc.Where(x => x.NomVendedor == vend).ToList();

                            if (vendedor.Count>0) 
                            {
                                double totalv = 0;
                                int totalaycsc = 0;
                                foreach (var itemv in vendedor) 
                                {
                                    totalv = (double)(totalv + _db2Context.Albventacabs.Where(x => x.Facturado == "T" && x.Fo == ids && x.Fecha.Value.Date >= ultimoDomingo.AddDays(-6).Date && x.Fecha.Value.Date <= ultimoDomingo.Date && x.Codvendedor == itemv.CodVendedor).Sum(t => t.Totalneto));
                                    ...
                                    using (SqlConnection connection = new SqlConnection(connectionString))
                                    {
                                        using (SqlCommand command = new SqlCommand("GET_AYCSCV", connection))
                                        {
                                            ...
                                            command.Parameters.AddWithValue("@IDS", ids);
                                            command.Parameters.AddWithValue("@CODV",itemv.CodVendedor);
                                ...
                                int incidencias = datas.Where(x => x.Vendedor == vend).ToList().Count();
                                datavendedores.Add(new { vendedor = vend, venta = totalv, aycsc = totalaycsc }); 
                            }
                        }
                    }
        ...
        [HttpPost]
        [Route("getExcelVentaBebidas")]
        public async Task<ActionResult> getExcelVentasBebidas([FromForm] string data)
        {
            try
            {
                List<ventasBebidasModel> datap = System.Text.Json.JsonSerializer.Deserialize<List<ventasBebidasModel>>(data);
                ...
                using (var package = new ExcelPackage())
                {
                    ...
                    for (int i = 1; i <= semanas.Length; i++) 
                    {
                        ...
                        worksheet.Cells[1, ic].Value = "VENTA SALÓN";
                        worksheet.Cells[1, ic].Style.Font.Bold = true;
                        worksheet.Cells[1, ic].Style.Fill.PatternType = ExcelFillStyle.Solid;
                        worksheet.Cells[1, ic].Style.Fill.BackgroundColor.SetColor(System.Drawing.Color.Black);
                        worksheet.Cells[1, ic].Style.Font.Color.SetColor(System.Drawing.Color.White);
                        ic++;
                        worksheet.Cells[1, ic].Value = "CERVEZAS";
                        worksheet.Cells[1, ic].Style.Font.Bold = true;
                        worksheet.Cells[1, ic].Style.Fill.PatternType = ExcelFillStyle.Solid;
                        worksheet.Cells[1, ic].Style.Fill.BackgroundColor.SetColor(System.Drawing.Color.Black);
                        worksheet.Cells[1, ic].Style.Font.Color.SetColor(System.Drawing.Color.White);
                        ic++;
                        worksheet.Cells[1, ic].Value = "REBELITROS";
                        worksheet.Cells[1, ic].Style.Font.Bold = true;
                        worksheet.Cells[1, ic].Style.Fill.PatternType = ExcelFillStyle.Solid;
                        worksheet.Cells[1, ic].Style.Fill.BackgroundColor.SetColor(System.Drawing.Color.Black);
                        worksheet.Cells[1, ic].Style.Font.Color.SetColor(System.Drawing.Color.White);
                    ...
                    foreach (int ids in sucursales) 
                    {
                        ...
                        foreach (int numsem in semanas)
                        {
                            ...
                            for (int i = 0; i < datasem.Count; i++)
                            {
                                ...
                                worksheet.Cells[ir, ic].Value = datasem[i].salon;
                                worksheet.Cells[ir, ic].Style.Numberformat.Format = "$#,##0.00";
                                ic++;

                                worksheet.Cells[ir, ic].Value = datasem[i].cerveza;
                                worksheet.Cells[ir, ic].Style.Numberformat.Format = "$#,##0.00";
                                ic++;

                                worksheet.Cells[ir, ic].Value = datasem[i].rebelitros;
                                worksheet.Cells[ir, ic].Style.Numberformat.Format = "$#,##0.00";
                                ic++;
                                ...
                                worksheet.Cells[ir, ic].Value = pcervezas;
                                worksheet.Cells[ir, ic].Style.Numberformat.Format = "0.00%";
                                if (pcervezas < .20)
                                {
        ...
        static List<(int semana, DateTime inicio, DateTime fin)> ObtenerSemanasYFechasEnRango(DateTime fechaInicial, DateTime fechaFinal)
        {
            ...
            DayOfWeek primerDiaSemana = DayOfWeek.Sunday;
